...
final class ClipboardWindowController: NSWindowController, NSWindowDelegate {
    ...
    private func makeClipboardView() -> ClipboardView {
        ClipboardView(
            clipboard: clipboard,
            drawerEdge: settings.clipboardDrawerEdge,
    ...
    private func restoredDrawerFrame(for window: NSWindow) -> NSRect? {
        let savedEdgeRaw = UserDefaults.standard.string(forKey: Self.drawerEdgeDefaultsKey)
        let savedEdge = savedEdgeRaw.flatMap(ClipboardDrawerEdge.init(rawValue:))
        guard savedEdge == settings.clipboardDrawerEdge else { return nil }
    ...
    private func persistDrawerFrameIfVisible() {
        ...
        UserDefaults.standard.set(settings.clipboardDrawerEdge.rawValue, forKey: Self.drawerEdgeDefaultsKey)
    }
