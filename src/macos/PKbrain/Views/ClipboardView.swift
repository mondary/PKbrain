...
struct ClipboardView: View {
    ...
    let drawerEdge: ClipboardDrawerEdge
    ...
    private var isVerticalDrawer: Bool { drawerEdge == .left || drawerEdge == .right }
    ...
    private var deck2SidebarViewport: some View {
        ...
        ScrollViewReader { proxy in
            ...
            .onChange(of: selectedID) { _ in scrollSelectionIntoView(proxy: proxy) }
            .onChange(of: entries.map(\.id)) { ids in
                if let selectedID, ids.contains(selectedID) { scrollSelectionIntoView(proxy: proxy) } else { selectedID = ids.first }
            }
        }
    }
    ...
    private func handleKeyDown(_ event: NSEvent) -> Bool {
        ...
        // Horizontal drawer: Left/Right. Vertical drawer: Up/Down.
        if event.keyCode == 123 || event.keyCode == 124 || event.keyCode == 125 || event.keyCode == 126 {
            let delta: Int
            if isVerticalDrawer {
                if event.keyCode == 126 { delta = -1 } // up
                else if event.keyCode == 125 { delta = 1 } // down
                else { return false }
            } else {
                if event.keyCode == 123 { delta = -1 } // left
                else if event.keyCode == 124 { delta = 1 } // right
                else { return false }
            }
...
private struct LabSettingsView: View {
    ...
    private var pluginsDirectoryURL: URL {
        storageRootURL.appendingPathComponent("Plugins", isDirectory: true)
    }
    ...
    private func createPluginTemplateIfMissing() {
        let fm = FileManager.default
        try? fm.createDirectory(at: pluginsDirectoryURL, withIntermediateDirectories: true)
        let manifestURL = pluginsDirectoryURL.appendingPathComponent("plugin-example.json")
